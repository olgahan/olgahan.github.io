...
{% for pub in pubs %}
- **{{ pub.title }}**
  {% if pub.coauthors and pub.coauthors != "" %} ({{ pub.coauthors }}){% endif %}, _{{ pub.venue }}_ ({{ pub.year }})  
  {% if pub.link %}[{{ pub.link_label | default: "PDF" }}]({{ pub.link }}){% endif %}
  {% if pub.abstract or pub.image %}
  <details>
    <summary>Show abstract & figure</summary>
...
{% for wp in wps %}
- **{{ wp.title }}**
  {% if wp.coauthors and wp.coauthors != "" %} ({{ wp.coauthors }}){% endif %}
