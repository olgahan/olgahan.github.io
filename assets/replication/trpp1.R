...
library(xtable)
getwd()
setwd("/Users/olgahan/Downloads/Dissertation3.17/tr")
#read data
...
df_tr_cace <- df_tr %>%
  mutate(
    passed_threat_check = (threat_v == "economy"  & mc_economy == "Ekonomi") |
      (threat_v == "security" & mc_security == "Güvenlik") |
      (threat_v == "culture"  & mc_culture == "Kültürel") |
      (threat_v == "general"  & mc_general == "Türkiye") |
      (threat_v == "placebo"),
    
    passed_attention = (attention1 == 4 & attention2 == "Vatanseverlik"),  # adjust if only 1
    
    complier = if_else(passed_threat_check & passed_attention, 1, 0)
  )
...
# Example for one treatment vs placebo (e.g., "security")
df_sub <- df_tr_cace %>%
  filter(threat_v %in% c("security", "placebo")) %>%
  mutate(
    Z = if_else(threat_v == "security", 1, 0),
    D = complier
  )

first_stage <- lm(D ~ Z, data = df_sub)
summary(first_stage)
...
df_tr = subset(df_tr, attention2 == "Vatanseverlik")

df_tr = subset(df_tr, citizenship == "Evet")

df_tr$Duration..in.seconds. = as.numeric(df_tr$Duration..in.seconds.)

#df_tr = subset(df_tr, Duration..in.seconds. > 179)
...
# df_tr <- df_tr %>%
#   mutate(across(contains("ns"), as.numeric)) %>%
#   rowwise() %>%
#   mutate(ns = mean(c_across(contains("ns") & !matches("citizenship")), na.rm = TRUE)) %>%
#   ungroup()
...
png(file="tr1_pp_dist.png", height = 1111, width = 1311,  res = 300)
grid.arrange(p1, p2, p3, p4, p5, p6, ncol = 2)
...
png(file="tr_bp_dist.png", height = 1111, width = 1311,  res = 300)
grid.arrange(bp1, bp2, bp3, bp4, bp5, bp6, ncol = 2)
...
png(file="tr_cp_dist.png", height = 1111, width = 1311,  res = 300)
grid.arrange(cp1, cp2, cp3, cp4, cp5, cp6, ncol = 2)
...
png(file="tr_ns_dist.png", height = 1111, width = 1311,  res = 300)
grid.arrange(ns1, ns2, ns3, ns4, ns5, ncol = 2)
...
data <- data %>% 
  mutate(party = as.factor(ifelse(party == "Hükûmet", 0,
                                  ifelse(party == "Muhalefet", 1, NA))),
         employment = as.factor(ifelse(employment == "Tam zamanlı çalışan/İşyeri sahibi", 1, 
                                       ifelse(employment == "İşsiz ve iş arayan", 0, NA) )),
         ideology = as.factor(ifelse(ideology == "Sağ", 0,
                                     ifelse(ideology == "Sol", 1, NA))),
         gender_respondent = as.factor(ifelse(gender_respondent == "Erkek", 0,
                                              ifelse(gender_respondent == "Kadın", 1, NA))),
         race = as.factor(ifelse(race=="Türk", 0, 1)),
         military = as.factor(ifelse(military == "Evet (uzun veya kısa dönem)", 1, 0))
  )
...
########
# Run regressions (diff-in-means as regression)

#######
# Run the regression
# Convert character columns to factors

table(df_tr$military_family)
#df_tr$military_binary = ifelse(df_tr$military == "Evet (uzun veya kısa dönem)", 1, 0)

df_tr$military_binary <- ifelse(df_tr$military == "Evet (uzun veya kısa dönem)", 1,
                                ifelse(df_tr$military == "Bedelli", 0, NA))

df_tr$family_military <- ifelse(df_tr$military_family == "Evet", 1,
                                ifelse(df_tr$military_family == "Hayır", 0, NA))
...
print(latex_table, type = "latex")
#xtable
clean_projected_patriotism_data <- function(df) {
  # Recode categorical variables
  df$race <- dplyr::recode(df$race,
                           "Çerkez" = "Circassian",
                           "Diğer" = "Other race",
                           "Kürt" = "Kurdish",
                           "Türk" = "Turkish"
  ...
  df$ideology <- dplyr::recode(df$ideology,
                               "Sağ" = "Right wing",
                               "Sol" = "Left wing"
  )
  
  df$party <- dplyr::recode(df$party,
                            "Hükûmet" = "Pro-government",
                            "Muhalefet" = "Opposition"
  )
  
  df$employment <- dplyr::recode(df$employment,
                                 "İşsiz fakat iş aramayan" = "Unemployed but not seeking jobs",
                                 "İşsiz ve iş arayan" = "Unemployed and seeking jobs",
                                 "Tam zamanlı çalışan/İşyeri sahibi" = "Employed full time/Self-employed",
                                 "Yarı zamanlı çalışan" = "Employed part-time"
  )
  
  df$threat_v <- dplyr::recode(df$threat_v,
                               "general" = "Threat (general)",
                               "culture" = "Threat (culture)",
                               "security" = "Threat (security)",
                               "economy" = "Threat (economy)"
  )
...
df_tr_clean <- clean_projected_patriotism_data(df_tr)
df_tr_clean20 <- clean_projected_patriotism_data(df_tr20)
df_tr_clean25 <- clean_projected_patriotism_data(df_tr25)
df_tr_clean30 <- clean_projected_patriotism_data(df_tr30)

names(df_tr_clean)
#MAIN MODEL
model <- lm(pp ~ cp + ns + bp   + military_binary + race + ideology + party + employment + threat_v, data = df_tr_clean)
...
model <- lm(pp ~ cp + ns + bp   + military_binary + race + ideology + party + employment + threat_v, data = df_tr_clean20)
...
model <- lm(pp ~ cp + ns + bp   + military_binary + race + ideology + party + employment + threat_v, data = df_tr_clean25)
...
model <- lm(pp ~ cp + ns + bp   + military_binary + race + ideology + party + employment + threat_v, data = df_tr_clean30)
...
# Fit a linear model
model <- lm(pp ~ cp + ns + bp +  military_binary + threat_v +  gender_respondent  + race + ideology + party + employment , data = df_tr)

# --- COEF PLOT: ONLY ns + military_binary (fixed recode) ---
...
tidy_two <- tidy(model, conf.int = TRUE) %>%
  ...
  mutate(term = ifelse(term == "ns", "National symbols", "Military service (binary)"))
...
ggsave("tr_coefplot_ns_military.png", p_two, width = 6.8, height = 3.6, dpi = 400)
...
model <- lm(pp ~ cp + ns + bp +  family_military + threat_v + gender_respondent  + race + ideology + party + employment + threat_v, data = df_tr)
...
# Fit a linear model, with alternative military
model <- lm(pp ~ cp + ns + bp +  family_military + threat_combined + gender_respondent  + race + ideology + party + employment + threat_v, data = df_tr)
